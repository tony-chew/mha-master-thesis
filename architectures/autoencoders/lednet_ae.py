...
CHANNELS = [32, 64, 128]
DOWN_CHANNELS = [64, 32, 3]
# DILATIONS = [[0, 1, 1, 1], [0, 1, 1], [0, 1, 2, 5, 9]]
...
class LEDNetBottleneck(torch.nn.Module):
    """
    The bottleneck for the LEDNet fast segmentation network outfitted as an autoencoder
    """
    def __init__(self,
                 input_res,
                 latent_size,
        ...
        """
        super(LEDNetBottleneck, self).__init__()
...
class LEDNetDecoder(torch.nn.Module):
    """
    The decoder for an outfitted autoencoder based on the LEDNet segmentation network
    """
    def __init__(self,
                 input_res,
                 latent_size,
        ...
        :param down_channels: the output channels of each downsampling stage of the encoder
        ...
        self.channels = down_channels
        self.dilations = [elem[::-1] for elem in dilations][::-1]
        self.dropout_rates = dropout_rates[::-1]
...
class LEDNetAE(torch.nn.Module):
    ...
    def __init__(self,
        ...
        :param down_channels: the output channels of each stage of the decoder
        :param dilations: the dilation rates of each layer of the encoder
        :param dropout_rates: the dropout rates of each layer of the encoder
        :param flatten_bottleneck: whether or not to apply flatten/linear operations to the bottleneck
        :param compress_bottleneck: whether or not to apply compression to the bottleneck
        ...
        self.bottleneck = LEDNetBottleneck(input_res=input_res,
                                           latent_size=latent_size,
                                           flatten_bottleneck=flatten_bottleneck,
                                           compress_bottleneck=compress_bottleneck,
                                           bottleneck_channels=bottleneck_channels)

        self.decoder = LEDNetDecoder(input_res=input_res,
                                     latent_size=latent_size,
                                     down_channels=down_channels,
                                     dilations=dilations,
                                     dropout_rates=dropout_rates,
